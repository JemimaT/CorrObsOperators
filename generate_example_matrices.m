% Generate example matrices using new functions - correlation matrices
...
% Check is implemented to guarantee the returned matrix is SPD and has
% minimum eigenvalue of at least 0.41


% pvec - prescribes the size of the diagonal blocks (off-diagonal blocks
% have conformable sizes). Length plen
%
% pdist - selection of on-diagonal block correlation structure (1 = SOAR is
% the only option currently available). Length = plen(plen+1)/2
%
...
% triangle of the matrix (including the on-diagonal blocks). Length = plen(plen+1)/2
%

pdist = ones([1,10]);
pcorr = [0.8,0,0,0,0.2,0,0,0.5,0,0.6]; 
